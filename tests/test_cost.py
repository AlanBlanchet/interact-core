...
def test_run_cost_estimate_unknown_count_must_match_its_nodes():
    known = NodeCostEstimate(node_id=uuid4(), known=True, low_usd=0.1, high_usd=0.2)
...
def test_run_cost_actual_sums_known_nodes_and_counts_unknown_ones():
    known = NodeCostActual(node_id=uuid4(), usage=NodeUsage(token_in=10, token_out=5), known=True, cost_usd=0.03, source=_source())
    unknown = NodeCostActual(node_id=uuid4(), usage=NodeUsage(calls=1), known=False)
    actual = RunCostActual(nodes=(known, unknown), total_usd=0.03, unknown_node_count=1)
